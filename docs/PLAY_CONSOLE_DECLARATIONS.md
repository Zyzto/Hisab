...
- **Content:** The policy text in `web/privacy/index.html` matches the in-app policy (Settings → Privacy Policy) and `assets/translations/en.json` / `ar.json` keys `privacy_policy_*` (including App Permissions). When updating the policy, change **en**, **ar**, and `web/privacy/index.html`.
...
- **Notification Listener:** If Play Console asks, declare it for the opt-in Android Transaction Scanner only (not FCM). Canonical behavior: [TRANSACTION_SCANNER.md](TRANSACTION_SCANNER.md); user-facing text: privacy App Permissions (`privacy_policy_permissions_body` / `web/privacy`).
