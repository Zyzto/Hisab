...
- **Content:** The policy text in `web/privacy/index.html` matches the in-app policy (Settings → Privacy Policy) and `assets/translations/en.json` keys `privacy_policy_*`, including the **App Permissions** section (Camera, Photos, Notifications / push, Notification access for Transaction Scanner on Android). When updating the policy, change both the app strings and `web/privacy/index.html`.
...
| **Other user content** (notification text for Transaction Scanner drafts) | Yes (Android only, if scanner enabled) | Parse bank/payment notifications into local draft expenses | No (processed and stored on device; draft tables are not synced) | Yes (opt-in; can disable / revoke Notification Listener access) |
...
- **Notification Listener (Android Transaction Scanner):** Separate from FCM push. When the user enables Transaction Scanner and grants **Notification access** in system settings, the app can read notification content from apps (optionally filtered by a sender whitelist) to propose draft expenses. Processing is on-device; confirmed expenses follow the normal expense path. See [TRANSACTION_SCANNER.md](TRANSACTION_SCANNER.md). In Play Console “Sensitive permissions” / Declarations, declare **Notification Listener** if asked, and state that it is used only for the opt-in Transaction Scanner.
