-- Per-user flag so bulk import sync can skip notify_group_activity → send-notification.
-- Client calls set_notify_suppress(true) around silent pending_writes push.

CREATE TABLE IF NOT EXISTS public.user_notify_suppress (
  user_id UUID PRIMARY KEY REFERENCES auth.users (id) ON DELETE CASCADE,
  active BOOLEAN NOT NULL DEFAULT FALSE,
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

ALTER TABLE public.user_notify_suppress ENABLE ROW LEVEL SECURITY;

CREATE POLICY user_notify_suppress_select_own
  ON public.user_notify_suppress
  FOR SELECT
  TO authenticated
  USING (user_id = auth.uid());

CREATE POLICY user_notify_suppress_upsert_own
  ON public.user_notify_suppress
  FOR ALL
  TO authenticated
  USING (user_id = auth.uid())
...
CREATE OR REPLACE FUNCTION public.set_notify_suppress(p_active BOOLEAN)
RETURNS VOID
LANGUAGE plpgsql
SECURITY INVOKER
...
BEGIN
  ...
  END IF;
  INSERT INTO public.user_notify_suppress (user_id, active, updated_at)
  VALUES (auth.uid(), COALESCE(p_active, FALSE), NOW())
...
REVOKE ALL ON FUNCTION public.set_notify_suppress(BOOLEAN) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.set_notify_suppress(BOOLEAN) TO authenticated;
